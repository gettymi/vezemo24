...
стоїть і чекає на завантаженні: погодинно + подача, мінімум 2 години.
Ставки тут не дублюємо: вони живуть у content/pricing.py.
Тому на цих сторінках немає ані кілометражу, ані готової суми — вона
чесно залежить від того, скільки триватиме завантаження.

Виняток — місто з "price": "km". На відстані під сотню кілометрів
погодинний рахунок перестає бути чесним: машина не стоїть на
завантаженні, вона їде. Такі напрямки рахуються за кілометром плюс
подача, і подача працює як мінімальна сума замовлення.
...
PLACES = [
    ...
    },
    {
        ...
        "ll": [49.7950, 30.1310],
        "side": "place.side.south",
        # Єдине місто в списку, яке рахується НЕ погодинно. Вісімдесят
        # кілометрів — це пів робочого дня самої дороги, і за годинником
        # виходить сума, якої ніхто не назве вголос. Власник сказав рахувати
        # за кілометром плюс подача, і подача тут — мінімум замовлення.
        # Прапорець читає templates/place.html; решта міст лишається на
        # годинах, і жодне число тут не зашите — усе з content/pricing.py.
        "price": "km",
        "copy": {
            "uk": {
